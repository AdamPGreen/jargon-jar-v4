...
export function TopSpendersLeaderboard({
  workspaceId,
  timePeriod,
}: TopSpendersLeaderboardProps) {
  const { data, isLoading, error } = useUserAmountLeaderboard({ workspaceId, timePeriod })

  return (
    <LeaderboardShell title="Top spenders" caption="Ranked by dollars filed" stamp="Most fined">
      {error ? (
        <LeaderboardError />
      ) : isLoading ? (
        <LeaderboardLoading />
